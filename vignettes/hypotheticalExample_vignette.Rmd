...
save(useGO, testSamples, universeGenes, file="inst/data/100GeneSamples.RData")
```

```{r runSamples}
# load("inst/data/100GeneSamples.RData")
testRes <- lapply(seq(1, length(testSamples)), function(x){
  print(x)
  sRes <- hyperGOMultiEnrichment(testSamples[[x]], universe=universeGenes)

  sP <- pvaluesMultiEnrich(c("s1", "s2"), useGO, sRes)
  sP <- pvalueDiffSig(sP, pCutoff=0.05, log=TRUE)
  sP$class <- sizeClass
  sP$iteration <- x
  return(sP)
})
