...
#' @param do.parallel use multiple cores to do everything
...
mmGeneSetTest <- function(index, statistics, ranks.only=FALSE, do.parallel=TRUE, ...){
  
  currCores <- getOption("mc.cores")
  if (is.null(currCores)){
    currCores <- 1
  }
  ...
  byIndex <- mclapply(index, function(inIndices){
  ...
  }, mc.preschedule=FALSE, mc.cores=currCores)
...
#' @importFrom parallel mclapply
multicontrastRomer <- function(index, y, design, contrastMatrix, nrot=9999, do.parallel=TRUE){
  ...
  outData <- mclapply(seq(1, nContrast), function(inCol){
