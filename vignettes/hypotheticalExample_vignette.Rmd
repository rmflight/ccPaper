...
tValues <- sample(tALL$t, length(universeGenes), replace=TRUE)
...
ggplot(tmpDat, aes(x=x, color=type)) + geom_histogram(binwidth=0.1, position="identity")

tmpComb <- data.frame(x=gseaTValues[["comb"]], type="all", stringsAsFactors=FALSE)
tmpComb$type[(universeGenes %in% gseaSamples[[1]])] <- "sample"
ggplot(tmpComb, aes(x=x, color=type)) + geom_histogram(binwidth=0.1, position="identity")
```
