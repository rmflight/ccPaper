...
deLung <- topTable(fitLung, coef="cancerIDTumor", number=Inf, adjust.method="BY", p.value=0.00001)
```
...
Lets actually compare this to the original distribution of fractions *vs* sizes

```{r compareFracs}
ALL_sizeFrac$genelist <- "all"
ALL_tmp <- rbind(ALL_sizeFrac, goFractions[,c("frac", "genelist", "size")])
ggplot(ALL_tmp, aes(x=size, y=frac, color=genelist)) + geom_point() + xlim(0, 1500)
```
