...
nGene <- 1000
sample1 <- sampleTerms(useGO, hsGO, 1000, 4)[1:nGene]
sample2 <- sampleTerms(useGO, hsGO, 1000, 4)[1:nGene]
```
...
sample1 <- c(sample1, noiseGenes)
sample2 <- c(sample2, noiseGenes)
```
...
```{r compRes}
s1_goP <- -1 * log10(pvalues(s1_enrich)[useGO])
s2_goP <- -1 * log10(pvalues(s2_enrich)[useGO])
```
...
com_goP <- -1 * log10(pvalues(com_enrich)[useGO])
```


### Compare Methods

To compare the methods, we will start with the `r length(useGO)` terms in our low, med and hi groups, and plot the p-values to see how they differe.

```{r compMethods}
plotP <- data.frame(pVal=c(s1_goP, s2_goP, com_goP), 
                    class=rep(c("s1", "s2", "s1.s2"), each=60))
plotP$nGene <- rep(useGO_counts, 3)
ggplot(plotP, aes(x=nGene, y=pVal, color=class)) + geom_point()
```