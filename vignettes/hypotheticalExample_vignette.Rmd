...
```{r gseaProportion}
gseaSamples <- function(inLimits, nSample){
  round(runif(nSample, inLimits[1], inLimits[2]))
}

gseaProportion <- function(inSize, useProp, statistics){
  statistics <- sort(statistics)
  nPosGene <- length(statistics)
  allProp <- runif(length(inSize), 0.1, 1)
  topLoc <- seq(1, round(nPosGene/2))
  botLoc <- seq(round(nPosGene/2)+1, nPosGene)
  gseaIndex <- lapply(seq(1, length(inSize)), function(index){
    nGene <- inSize[index]
    sigProp <- allProp[index]
    
    nTop <- round(nGene*sigProp)
    nBot <- nGene - nTop
    
    useIndex <- c(sample(topLoc, nTop), sample(botLoc, nBot))
    return(useIndex)
  })
  gseaData <- list(index=gseaIndex, size=inSize, frac=allProp, statistics=statistics)
}
```
