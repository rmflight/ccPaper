...
This function generates for each term of given size a set of random indices into the statistics, and then reshuffles the statistics to put the  proper proportion of genes into the top and bottom of the ranked statistics. So we are basically considering a set of statistics  for each term and sample.
...
```{r gseaVaryProp}
# in this case, sigProp is a list, one list for each sample
gseaProportion2 <- function(inSize, sigProp, statistics){
  statistics <- sort(statistics)
  nPosGene <- length(statistics)
...
```{r checkgseaProportions2}
testSize <- gseaSizeRange(c(500,500), 50)
useStatistics <- runif(10000, -6, 6)
useSig <- list(s1=runif(50, .1, 1), s2=runif(50, .1, 1))
testStats <- gseaProportion2(testSize, useSig, useStatistics)
...
So, none of this has really worked, at least not in the way we hoped. 
...
Verify that using p-values and changing proportion in the top will actually work.

For this to work with `geneSetTest`, we need to make the range 0, 0.5, 1 conform to -1, 0, 1.  Let's use a linear regression.
...
So our transformation works. Lets make sure it will work in our other data.
...
```{r }
gseaProportionFisher <- function(inSize, sigProp, statistics){
  statistics <- sort(statistics)
  nPosGene <- length(statistics)
  nSample <- length(sigProp)
  grpLoc <- seq(1, nPosGene, round(nPosGene / 4))
...
Other options to think about: use maximum p-value to do the combining (as tseng did), and using ranks only.
