...
testStats_clean$class <- sizeClass
testStats_clean$frac <- goFractions$frac[1:length(sizeClass)]
...
noiseCleanDiff$class <- sizeClass
noiseCleanDiff$frac <- goFracNoise$frac[1:length(sizeClass)]
...
save(noiseSamples, cleanSample, sizeNoise, fracShared, universeGenes, useGO, sizeClass, file=file.path(useDir, 'inst/data/noiseSamples.RData'))
```
...
sfLibrary(org.Hs.eg.db)
sfLibrary(GO.db)
sfExport("universeGenes", "useGO", "sizeClass", "cleanSample")
...
```{r gseaSamples}
allGenes <- unique(unlist(hsGO, use.names=FALSE))
gseaPVals <- runif(length(allGenes), min=0.18, max=0.22)
samplePValRange <- runif(10000, min=0.005, max=0.015)
names(gseaPVals) <- allGenes

gseaSamples <- list(s1 = sampleTerms(useGO, hsGO, 1000, 4),
                    s2 = sampleTerms(useGO, hsGO, 1000, 4))
gseaSampleVals <- samplePValueGenGSEA(gseaSamples, gseaPVals, samplePValRange)

gseaSetResults <- multiSampleGeneSetTest(gseaSampleVals, hsGO[useGO], alternative="mixed")
```
