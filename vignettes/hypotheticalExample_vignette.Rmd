...
ggplot(hsGO_count, aes(x=size)) + geom_bar(binwidth=10) + xlim(0, 2000) + ylim(0, 500) + useTheme
```
...
ggplot(goFractions, aes(x=size, y=frac, color=genelist)) + geom_point() + useTheme
```
...
ggplot(ALL_tmp, aes(x=size, y=frac)) + geom_point() + xlim(0, 1500) + facet_grid(genelist ~ .) + useTheme
```
...
ggplot(outMedians, aes(x=size, y=med, color=shared)) + geom_point() + useTheme
ggplot(outMedians, aes(x=size, y=low, color=shared)) + geom_point() + useTheme
```
